...
火车票管理系统为组队作业，共11组，每组由两名A班同学和一名B班同学共三人组成。
...
- `name`：由二至五个汉字组成的字符串。
- `mailAddr`：同一般邮箱地址格式，且仅含数字、大小写字母，`@`和`.`。
- `priority`：所处用户组优先级，为一个0~10的整数。
...
- [N] `add_user`
  ...
  - 说明
    ...
    创建一个`<username>`为`-u`，`<password>`为`-p`，`<name>`为`-n`，`<mailAddr>`为`-m`，`<priority>`为`-g`的用户。
...
- [SF] `query_profile`
  ...
  - 返回值
    ...
    查询成功：一行字符串，依次为被查询用户的`<username>`，`<name>`，`<mailAddr>`和`<priority>`，用一个空格隔开。
